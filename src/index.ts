...
export default class Pluggable<Context> {
   ...
   * @memberof Pluggable
   */
  onEveryOnce(events: string[], callback: Function, detail?: ListenerDetail) {
    Promise.all(events.map(event => new Promise(rs => this.once(event, rs, detail))))
      .then(datas => callback.call(this, datas))
  }
   ...
   * @memberof Pluggable
   */
  onAnyOnce(events: string[], callback: Function, detail?: ListenerDetail) {
    Promise.race(events.map(event => new Promise(rs => this.once(event, rs, detail))))
      .then(datas => callback.call(this, datas))
  }
